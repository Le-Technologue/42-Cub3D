...
static void	normalize_wall_dist(t_fov *fov, t_ray *ray)
{
	if (ray->side == EAS || ray.side == WES)
		ray->wall_dist = (ray->reached.x - fov->plyr.pos.x
				+ (1 - ray->step.x) / 2) / ray->dir.x;
	else
		ray->wall_dist = (ray->reached.y - fov->plyr.pos.y
				+ (1 - ray->step.y) / 2) / ray->dir.y;
}

static void	dda(t_map *map, t_ray *ray)
{
...
static void	reach_grid(t_plyr plyr, t_ray *ray)
{
	if (ray->dir.x < 0)
	{
		ray->step.x = -1;
